...
class UnicodeTypeDB {
    ...
    static final short[] index1 = readIndex("unicode_db/index1.bin");
    static final short[] index2 = readIndex("unicode_db/index2.bin");

    private static short[] readIndex(String name) {
        try (DataInputStream dis = new DataInputStream(UnicodeTypeDB.class.getResourceAsStream(name))) {
            final short[] index = new short[dis.readInt()];
            for (int i = 0; i < index.length; i++) {
                index[i] = dis.readShort();
            }
            return index;
        } catch (IOException e) {
            throw new IOError(e);
